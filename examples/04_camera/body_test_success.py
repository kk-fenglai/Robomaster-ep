...
def adjust_chassis_movement(distance):
    # 根据目标距离调整小车移动，这里只是示例，您可以根据具体需求调整移动逻辑
    if distance > 3:  # 如果目标距离小于1米
        ep_chassis.drive_speed(x=0.1, y=0, z=0)  # 向前移动
    elif distance <3:  # 如果目标距离大于2米
        ep_chassis.drive_speed(x=-0.1, y=0, z=0)  # 向后移动
